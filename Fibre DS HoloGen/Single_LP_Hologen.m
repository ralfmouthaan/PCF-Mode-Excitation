...
pause(0.1);
RetVal = DirectSearchSymmetryBinary(Illum, Target, Mask);

%% Plot hologram
figure;
imagesc(angle(RetVal.Holo));
title('Hologram');
axis square;
...
figure;
subplot(1,2,1);
imagesc(u*1e6, u.'*1e6, abs(RetVal.Replay));
axis square
title('|Replay|');
xlabel('\mum'); ylabel('\mum');
xlim([-35 35]);
ylim([-35 35]);
subplot(1,2,2);
imagesc(u*1e6, u.'*1e6, angle(RetVal.Replay));
axis square
title('\angleReplay');
xlabel('\mum'); ylabel('\mum');
xlim([-35 35]);
